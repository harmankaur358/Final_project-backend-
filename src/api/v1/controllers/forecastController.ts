 ...
 * @param req.params.id - The forecast ID
 * @returns {Promise<Response>}
 */
export const getForecastById = async (
  ...
  try {
    ...
    if (!id) {
      return res.status(400).json(errorResponse("Forecast ID is required"));
    }

    const forecast: Forecast | null = await forecastService.getForecastById(id);

    if (!forecast) {
      return res.status(404).json(errorResponse("Forecast not found"));
    }
 ...
 * @param req.params.locationId - The location ID to filter forecasts
 * @returns {Promise<Response>}
 */
export const getForecastsByLocation = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<Response | void> => {
  try {
    ...
    if (!forecasts || forecasts.length === 0) {
      return res.status(404).json(errorResponse("No forecasts found for this location"));
    }
 ...
 * @param req.body - Forecast creation data
 * @returns {Promise<Response>}
 */
export const createForecast = async (
  ...
  try {
    const { locationId, temperature, humidity, windSpeed, date } = req.body;
 ...
 * @param req.params.id - The forecast ID
 * @param req.body - Forecast update data
 * @returns {Promise<Response>}
 */
 ...
 * @param req.params.id - The forecast ID to delete
 * @returns {Promise<Response>}
 */
